...
const char* get_mcc134_error_message(int error_code)
{
    const char* msg;
    ...
    else
    {
        ...
        switch (error_code)
        {
            case NO_HAT_DEVICES_FOUND:
                msg = (const char*)"No MCC 134 Hat devices were found.";
                break;
...
// thread.
void show_mcc134_error_main_thread(gpointer error_code_ptr)
{
    int error_code = *(int*)error_code_ptr;
    show_mcc134_error(error_code);
}


// Get the error message for the specified error code, and display it in a
// dialog box.
void show_mcc134_error(int error_code)
{
    // Get the error message.
    const char* errmsg = get_mcc134_error_message(error_code);
