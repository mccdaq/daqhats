...
GtkWidget *labelFile, *dataBox, *fftBox, *rbContinuous, *rbFinite, *spinRate,
          *comboBoxFftSize, *btnSelectLogFile, *chkChan[MAX_172_CHANNELS],
          *chkIepe, *spinSensitivity, *btnStart_Stop;
GMutex data_mutex;
pthread_t threadh;
pthread_mutex_t graph_init_mutex;
pthread_cond_t graph_init_cond;
...
GraphChannelInfo graphChannelInfo[MAX_172_CHANNELS];

// Function Prototypes
static void* read_and_display_data ();
static int open_first_hat_device(uint8_t* hat_address);
static int allocate_channel_xy_arrays();
static int create_selected_channel_mask();
static void set_enable_state_for_controls(gboolean state);
static gboolean refresh_graph(int* start_sample);
static void copy_data_to_xy_arrays(double* hat_read_buf,
    int read_buf_start_index, int channel, int stride, int buffer_size_samples,
...
static void sensitivity_changed_handler(GtkWidget *widget, gpointer user_data);
...
int main(void)
{
    int retval = 0;
    GtkApplication *app;
    GdkRGBA legendColor[MAX_172_CHANNELS];

    // Set the default filename.
    getcwd(csv_filename, sizeof(csv_filename));
    strcat(csv_filename, "/log_files/data.csv");
...
static void copy_data_to_xy_arrays(double* display_buf,
                                   ...
                                   int stride, int buffer_size_samples,
                                   uint32_t start_sample)
{
...
static gboolean refresh_graph(int* start_sample_ptr)
{
    ...
    gfloat end = (gfloat)(start_sample + g_fft_size);
...
static gboolean initialize_graphs() {
    ...
    while (chanMask > 0)
    {
        ...
        if (chanMask & 1)
        {
            graphChannelInfo[channel].graph = gtk_databox_lines_new
                ((guint)g_fft_size, graphChannelInfo[channel].X,
            ...
            graphChannelInfo[channel].fft_graph = gtk_databox_lines_new
                ((guint)g_fft_size/2 + 1, graphChannelInfo[channel].fft_X,
...
static void * read_and_display_data ()
{
	...
	retval = init_log_file(log_file_ptr);
	if (retval < 0)
	{
    ...
    while (g_done != TRUE)
    {
        ...
        if(!g_continuous) {
            samples_to_read = (g_fft_size - sample_count) * num_channels;
        }
        ...
        start_sample = sample_count >= g_fft_size ? (sample_count-g_fft_size) : 0;
        ...
        if(!g_continuous && sample_count == g_fft_size) {
            g_main_context_invoke(context, (GSourceFunc)stop_scan, NULL);
        }
...
static void start_stop_event_handler(GtkWidget *widget, gpointer data)
{
    ...
    if (strcmp(StartStopBtnLbl , "Start") == 0)
    {
        ...
        retval = mcc172_a_in_scan_start(g_hat_addr, g_chan_mask,
            10 * MAX_172_CHANNELS * g_sample_rate, options);
        if(retval == RESULT_SUCCESS)
        {
...
static void set_y_axis_range()
{
    ...
    // Calculate the min and max y values for the plot
    yMin = -6000.0 / g_sensitivity * g_zoom_level;
    yMax = 6000.0 / g_sensitivity * g_zoom_level;
    // Set the range
...
static void app_activate_handler(GtkApplication *app, gpointer user_data)
{
    ...
    gtk_css_provider_load_from_data (GTK_CSS_PROVIDER (cssProvider),
        "#Chan0 block.filled {background-color: rgba(221, 50, 34, 1); "
            "border-color: rgba(221, 50, 34, 1);}\n"
        "#Chan1 block.filled {background-color: rgba(52, 130, 203, 1); "
            "border-color: rgba(52, 130, 203, 1);}\n"
        "#startStop.circular {border-color: #3B5998; background-color: #3B5998;}\n",
         -1, NULL);
